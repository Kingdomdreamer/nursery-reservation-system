...
export default function AdminDashboard({ onPageChange }: AdminDashboardProps) {
  ...
  const handleFormConfigurationCheck = async () => {
    setCheckingForms(true)
    setShowFormCheckModal(true)
    
    try {
      const result = await FormService.checkFormConfiguration()
      setFormCheckResult(result)
    } catch (error) {
      console.error('フォーム設定確認中にエラー:', error)
      setFormCheckResult({
        totalForms: 0,
        activeForms: 0,
        inactiveForms: 0,
                ...
                {upcomingTasks.map((task, index) => (
                  <div key={index} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                    ...
                    <div className="flex items-center gap-2">
                      {task.task === 'フォーム設定の確認' && (
                        <button
                          onClick={handleFormConfigurationCheck}
          ...
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-lg font-semibold">フォーム設定確認結果</h3>
                <button
              ...
              {checkingForms ? (
                <div className="text-center py-8">
                  <Icon icon={Icons.loading} size="xl" className="animate-spin mx-auto mb-4 text-blue-600" />
                  <p className="text-gray-600">フォーム設定を確認しています...</p>
                </div>
              ) : formCheckResult && (
                <div className="space-y-6">
                  {/* 統計情報 */}
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="bg-blue-50 p-4 rounded-lg text-center">
                      <div className="text-2xl font-bold text-blue-600">{formCheckResult.totalForms}</div>
                      <div className="text-sm text-gray-600">総フォーム数</div>
                    </div>
                    <div className="bg-green-50 p-4 rounded-lg text-center">
                      <div className="text-2xl font-bold text-green-600">{formCheckResult.activeForms}</div>
                      <div className="text-sm text-gray-600">アクティブ</div>
                    </div>
                    <div className="bg-gray-50 p-4 rounded-lg text-center">
                      <div className="text-2xl font-bold text-gray-600">{formCheckResult.inactiveForms}</div>
                      <div className="text-sm text-gray-600">非アクティブ</div>
                    </div>
                    <div className="bg-red-50 p-4 rounded-lg text-center">
                      <div className="text-2xl font-bold text-red-600">{formCheckResult.expiredForms}</div>
                      <div className="text-sm text-gray-600">期限切れ</div>
                    </div>
                  </div>

                  {/* 問題点 */}
                  {formCheckResult.issuesFound.length > 0 && (
                    <div>
                      <h4 className="font-medium text-red-700 mb-3 flex items-center">
                        <Icon icon={Icons.warning} size="sm" className="mr-2" />
                        問題点 ({formCheckResult.issuesFound.length}件)
                      </h4>
                      <div className="space-y-2">
                        {formCheckResult.issuesFound.map((issue: string, index: number) => (
                  ...
                  {formCheckResult.recommendations.length > 0 && (
                    <div>
                      <h4 className="font-medium text-yellow-700 mb-3 flex items-center">
                        <Icon icon={Icons.info} size="sm" className="mr-2" />
                        推奨事項 ({formCheckResult.recommendations.length}件)
                      </h4>
                      <div className="space-y-2">
                        {formCheckResult.recommendations.map((recommendation: string, index: number) => (
                          <div key={index} className="bg-yellow-50 border border-yellow-200 p-3 rounded text-sm text-yellow-700">
                            {recommendation}
                          </div>
                        ))}
                      </div>
                  ...
                  {/* 正常な場合 */}
                  {formCheckResult.issuesFound.length === 0 && formCheckResult.recommendations.length === 0 && (
                    <div className="bg-green-50 border border-green-200 p-4 rounded text-center">
                      <Icon icon={Icons.success} size="lg" className="mx-auto mb-2 text-green-600" />
                      <p className="text-green-700 font-medium">フォーム設定に問題はありません</p>
                      <p className="text-green-600 text-sm mt-1">すべてのフォームが正常に設定されています</p>
                    </div>
                  )}
                </div>
              )}
              ...
              <div className="mt-6 flex justify-end space-x-3">
                ...
                {formCheckResult && (
                  <button
                    onClick={() => onPageChange?.('form-builder')}
